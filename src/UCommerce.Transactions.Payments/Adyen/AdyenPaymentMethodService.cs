...
using UCommerce.Transactions.Payments.Adyen.Test.ModificationSoapService;
using UCommerce.Transactions.Payments.Adyen.Test.RecurringSoapService;
using UCommerce.Transactions.Payments.Common;
using UCommerce.Web;
using Amount = UCommerce.Transactions.Payments.Adyen.Test.ModificationSoapService.Amount;
using Recurring = UCommerce.Transactions.Payments.Adyen.Test.ModificationSoapService.Recurring;
	...
	public class AdyenPaymentMethodService : ExternalPaymentMethodService
	{
		...
		private void AuthorizeRecurringPaymentInternal(Payment payment, out string status)
		{
			...
			var result = PaymentClient.authorise(
				new Test.ModificationSoapService.PaymentRequest
				{
		...
		protected void SaveRecurringDetailReference(Payment payment)
		{
			try
			{
				var result = RecurringClient.listRecurringDetails(new RecurringDetailsRequest
				{
					...
					recurring = new Test.RecurringSoapService.Recurring
					{
						contract = "RECURRING"
					},
					merchantAccount = payment.PaymentMethod.DynamicProperty<string>().MerchantAccount
